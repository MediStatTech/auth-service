...
import (
	...
	"github.com/MediStatTech/auth-service/internal/transport/grpc/jwt"
	"github.com/MediStatTech/auth-service/internal/transport/grpc/positions"
	"github.com/MediStatTech/auth-service/internal/transport/grpc/staffs"
...
func New(p *pkg.Facade, appInstance *app.Facade) (*Server, error) {
	...
	staffsHandler := staffs.New(opts)
	positionsHandler := positions.New(opts)
	jwtHandler := jwt.New(opts)

	auth_pb.RegisterJwtServiceServer(server, jwtHandler)
	auth_pb.RegisterStaffsServiceServer(server, staffsHandler)
	auth_pb.RegisterPositionServiceServer(server, positionsHandler)
